...
import Algorithm.Marchant.Moteur.Calcul;
...
public class Algorithme {
            ...
            if (rdvDuJour.size() != 0) {
                for (Appointement rdvDejaPresent : rdvDuJour) {
                    ...
                    if (rdvDejaPresent.equalsPosition(positionApresAlgo[i])) { // positif si la position correspond au
                        ...
                        rdvReturn.add(new Appointement(momentOuOnRegarde.toLocalDate(), momentOuOnRegarde.toLocalTime(),
    ...
    public List<Appointement> avoirLesRendezVousDejaDonnee(LocalDateTime dateDemander, Doctor docteurChoisit) {
        return AccesStockageInformation.getRendezVousDuJour(dateDemander, docteurChoisit);
    }

    public List<Appointement> rendezvousDejaDonneeAPartirDunMoment(LocalDateTime dateDemander, Doctor docteurChoisit) {
        List<Appointement> retour = new ArrayList<Appointement>();
        for (Appointement rendezVous : AccesStockageInformation.getRendezVousDuJour(dateDemander, docteurChoisit)) {
